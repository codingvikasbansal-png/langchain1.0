...
const generatePieChart = tool(
  ...
  },
  {
    ...
    description:
      "Generate pie chart data. User will give categories + values. Extract them cleanly.",
    schema: z.object({
      labels: z
        .array(z.string())
        .describe(
          "Labels for pie chart slices. Example: ['Apples', 'Bananas']"
        ),
      values: z
        .array(z.number())
        .describe(
          "Numeric values for each label. Must match the labels count. Example: [10, 20]"
        ),
    }),
  }
);
...
const generateTable = tool(
  ...
  },
  {
    name: "generate_table",
    description:
      "Generate a data table with columns and rows. User will provide tabular data. Extract column headers and row data cleanly.",
    schema: z.object({
      columns: z
        .array(z.string())
        .describe(
          "Column headers for the table. Example: ['Name', 'Age', 'City']"
        ),
      rows: z
        .array(z.record(z.any()))
        .describe(
          "Array of row objects where keys are column names. Example: [{ 'Name': 'John', 'Age': 30, 'City': 'NYC' }]"
        ),
    }),
  }
);
